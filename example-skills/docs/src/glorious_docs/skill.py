...
def create(
    ...
    existing = _ctx.conn.execute(
        "SELECT id FROM docs_documents WHERE id = ?", (doc_id,)
    ).fetchone()
...
def update(
    ...
    row = _ctx.conn.execute(
        "SELECT id, title, content, epic_id, version FROM docs_documents WHERE id = ?",
        (doc_id,)
    ).fetchone()
...
def export_doc(
    ...
    if version:
        ...
        row = _ctx.conn.execute(
            "SELECT id, content FROM docs_documents WHERE id = ?",
            (doc_id,)
        ).fetchone()
...
def versions(
    ...
    if json_output:
        typer.echo(json.dumps([
            {"version": row[0], "changed_at": str(row[1]), "size": row[2]}
            for row in vers
        ]))
    else:
        if not vers:
