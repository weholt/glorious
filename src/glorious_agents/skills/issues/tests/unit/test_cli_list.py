...
class TestIssuesList:
    ...
    def test_list_filter_priority_range(self, cli_runner: CliRunner):
        ...
        # Filter by range 0-2
        result = cli_runner.invoke(app, ["list", "--priority-min", "0", "--priority-max", "2", "--json"])
        assert result.exit_code == 0
        data = json.loads(result.stdout)
        assert len(data) == 3
    ...
    def test_list_filter_by_labels_and(self, cli_runner: CliRunner):
        ...
        result = cli_runner.invoke(app, ["list", "--label", "bug,critical"])
    ...
    def test_list_filter_by_labels_or(self, cli_runner: CliRunner):
        ...
        result = cli_runner.invoke(app, ["list", "--label-any", "bug,feature"])
