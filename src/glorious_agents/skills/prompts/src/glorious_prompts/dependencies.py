"""Dependency injection for prompts skill.
...
def get_engine() -> Engine:
    """Get database engine for prompts skill.
...
def get_prompts_service(
    engine: Engine | None = None, event_bus: EventBus | None = None
) -> PromptsService:
    """Get prompts service with dependencies.
    ...
    Returns:
        Configured PromptsService instance
    """
    if engine is None:
        engine = get_engine()
    ...
    return PromptsService(uow, event_bus)
