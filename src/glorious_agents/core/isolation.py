...
class RestrictedSkillContext:
    ...
    def cache_set(self, key: str, value: Any, ttl: int | None = None) -> None:
        """Set value in cache."""
        return self._ctx.cache_set(key, value, ttl)
    
    def cache_has(self, key: str) -> bool:
        """Check if key exists in cache."""
        return self._ctx.cache_has(key)
    
    def cache_delete(self, key: str) -> bool:
        """Delete key from cache."""
        return self._ctx.cache_delete(key)
    
    def cache_clear(self) -> None:
        """Clear cache."""
        return self._ctx.cache_clear()
    
    def cache_prune_expired(self) -> int:
        """Prune expired cache entries."""
        return self._ctx.cache_prune_expired()
